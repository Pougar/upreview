...
export async function POST(req: Request) {
  try {
    ...
    // 2) Get the sender's template + names
    ...
    const html = `
      ...
      <div style="margin:24px 0;">
        <!-- Good Review button -->
        <a href="http://localhost:3000/submit-review/${clientId}?type=good"
        ...
        <!-- Bad Review button -->
        <a href="http://localhost:3000/submit-review/${clientId}?type=bad"
    ...
    // 5) Send email via Resend (only proceed if send succeeds)
    const data = await resend.emails.send({
      from: `${senderName} <onboarding@resend.dev>`, // replace once your domain is verified
      to: [client.email],
    ...
    // 6) Atomically mark email_sent = TRUE and log the action (only on first transition)
    const db = await pool.connect();
    try {
      await db.query("BEGIN");

      // Only flip if not already true (protects against duplicates/races)
      ...
      if ((upd.rowCount ?? 0) > 0) {
        // First time we set it true -> log the action
        await db.query(
